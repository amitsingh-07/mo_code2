    ...
    if(this.endowmentSkipEnable){
      ...
      form.value.endowmentPlan.forEach((preferenceDetails: any, index) => {
        ...
        if(preferenceDetails.endowmentplanShow == true){  
            let aboutAgeCal =this.aboutAge.getAboutAge(preferenceDetails.age, (preferenceDetails.gender == 'Male')?this.translate.instant('CMP.ENDOWMENT_PLAN.MALE_ABOUT_YEAR'):this.translate.instant('CMP.ENDOWMENT_PLAN.FEMALE_ABOUT_YEAR'));          
            dependantArray.push({
      ...
      this.summaryModalDetails = { setTemplateModal: 1, dependantModelSel: true, contentObj: childrenEducationDependantModal, dependantDetails: dependantArray, nextPageURL: (COMPREHENSIVE_ROUTE_PATHS.STEPS)+'/2' };
      this.comprehensiveService.openSummaryPopUpModal(this.summaryModalDetails);
    }
  }
  showToolTipModal() {
