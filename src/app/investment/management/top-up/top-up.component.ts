...
export class TopUpComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.cashBalance = this.managementService.getUserCashBalance();
  ...
  getTopupInvestmentList() {
    this.managementService.getTopupInvestmentList().subscribe((data) => {
  ...
  selectedInvestment(investmenttype) {
    ...
    this.managementService.setInvestmentValue(this.investment.value);
  ...
  buildFormInvestment() {
    if (this.formValues.Investment === MANAGEMENT_CONSTANTS.TOPUP.MONTHLY_INVESTMENT) {
  ...
  setOnetimeMinAmount(data) {
    if (this.formValues.Investment === 'Monthly Investment') {
      this.managementService.setInvestmentValue(data[1].value);
    } else {
      this.managementService.setInvestmentValue(data[0].value);
    }
  }
  goToNext(form) {
    if (!form.valid) {
      ...
      const error = this.managementService.doFinancialValidations(form, allowZero);
  ...
  saveFundingDetails() {
    ...
    this.managementService.setFundingDetails(topupValues);
  }

  getMonthlyInvestmentInfo() {
    this.managementService.getMonthlyInvestmentInfo().subscribe((response) => {
  ...
  getOneTimeInvestmentInfo() {
    this.managementService.getOneTimeInvestmentInfo().subscribe((response) => {
      if (response.responseMessage.responseCode >= 6000) {
