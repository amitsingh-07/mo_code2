...
export class WithdrawalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.portfolioList = this.managementService.getUserPortfolioList();
    this.cashBalance = parseFloat(this.decimalPipe.transform(this.managementService.getUserCashBalance(), '1.0-2').replace(/,/g, ''));
    this.translateParams = {
      MIN_WITHDRAW_AMOUNT: MANAGEMENT_CONSTANTS.WITHDRAW.MIN_WITHDRAW_AMOUNT,
      MIN_BALANCE_AMOUNT: MANAGEMENT_CONSTANTS.WITHDRAW.MIN_BALANCE_AMOUNT
    };
    this.entitlements = this.managementService.getEntitlementsFromPortfolio(this.portfolioList[0])
    this.buildForm();
  }
  ...
  buildForm() {
    ...
    this.withdrawForm.get('withdrawType').valueChanges.subscribe((value) => {
      ...
      if (value) {
        ...
        setTimeout(() => {
          switch (value.id) {
            case MANAGEMENT_CONSTANTS.WITHDRAW.PORTFOLIO_TO_CASH_TYPE_ID:
              this.buildFormForPortfolioToCash();
              this.isFromPortfolio = true;
              break;
            case MANAGEMENT_CONSTANTS.WITHDRAW.PORTFOLIO_TO_BANK_TYPE_ID:
              this.buildFormForPortfolioToBank();
              this.isFromPortfolio = true;
              break;
            case MANAGEMENT_CONSTANTS.WITHDRAW.CASH_TO_BANK_TYPE_ID:
    ...
    this.withdrawForm.get('withdrawPortfolio').valueChanges.subscribe((value) => {
      if (value) {
        ...
        this.isRedeemAll = (roundOffValue <
                          (MANAGEMENT_CONSTANTS.WITHDRAW.MIN_WITHDRAW_AMOUNT + MANAGEMENT_CONSTANTS.WITHDRAW.MIN_BALANCE_AMOUNT)
                          && roundOffValue > 0);
        this.withdrawForm.addControl(
            'withdrawAmount',
            new FormControl( {
  ...
  buildFormForCashToBank() {
    this.isRedeemAll = ( this.cashBalance < MANAGEMENT_CONSTANTS.WITHDRAW.MIN_WITHDRAW_AMOUNT && this.cashBalance > 0 );
    this.withdrawForm.addControl(
      'withdrawAmount',
      new FormControl({
  ...
  getLookupList() {
    this.withdrawalTypes = MANAGEMENT_CONSTANTS.WITHDRAW.WITHDRAWAL_TYPES;
  }
  ...
  showConfirmWithdrawModal(form) {
    ...
    ref.componentInstance.confirmed.subscribe(() => {
      ref.close();
      this.managementService.setWithdrawalTypeFormData(form.getRawValue(), this.isRedeemAll);
      this.saveWithdrawal();
      // confirmed
    });
  ...
  goToNext(form) {
    if (!form.valid) {
      ...
      const error = this.managementService.getFormErrorList(form);
      const ref = this.modal.open(ErrorModalComponent, { centered: true });
      ...
      if (
        ...
        MANAGEMENT_CONSTANTS.WITHDRAW.CASH_TO_BANK_TYPE_ID ||
        form.value.withdrawType.id ===
        MANAGEMENT_CONSTANTS.WITHDRAW.PORTFOLIO_TO_BANK_TYPE_ID
      ) {
        this.router.navigate([MANAGEMENT_ROUTE_PATHS.WITHDRAWAL_PAYMENT_METHOD]);
      } else {
        this.showConfirmWithdrawModal(form);
      }
    }
  ...
  withdrawAmountValidator(balance, source): ValidatorFn {
    ...
    return (control: AbstractControl) => {
      if (control) {
        ...
        } else if ((source === 'PORTFOLIO') && (balance - userInput >= MANAGEMENT_CONSTANTS.WITHDRAW.MIN_BALANCE_AMOUNT)) { // Minimum Withdrawal Check
          if (userInput < MANAGEMENT_CONSTANTS.WITHDRAW.MIN_WITHDRAW_AMOUNT) {
        ...
        } else if ((source === 'PORTFOLIO') && (balance - userInput < MANAGEMENT_CONSTANTS.WITHDRAW.MIN_BALANCE_AMOUNT) && (userInput != balance) ) { // Minimum Balance Check
