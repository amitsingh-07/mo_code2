...
export class InvestmentOverviewComponent implements OnInit {
  ...
  yourPortfolio(portfolio) {
    if (portfolio.portfolioStatus !== 'EXPIRED') {
      this.managementService.setPortfolioValues(portfolio);
      this.managementService.setHoldingValues(portfolio.dpmsDetailsDisplay);
      this.router.navigate([MANAGEMENT_ROUTE_PATHS.YOUR_PORTFOLIO]);
    }
  }
  getInvestmentOverview() {
    this.managementService.getInvestmentOverview().subscribe((data) => {
      if (data.responseMessage.responseCode >= 6000) {
        ...
        this.managementService.setUserPortfolioList(this.portfolioList);
        if (this.investmentoverviewlist.data.cashAccountDetails) {
          this.managementService.setUserCashBalance(
            this.investmentoverviewlist.data.cashAccountDetails.availableBalance
          );
        }
  ...
  deletePortfolio(portfolio) {
    ...
    ref.componentInstance.yesClickAction.subscribe(() => {
      this.managementService.deletePortfolio(portfolio).subscribe((data) => {
        if (data.responseMessage.responseCode < 6000) {
          if (
            data.objectList &&
            data.objectList.length &&
