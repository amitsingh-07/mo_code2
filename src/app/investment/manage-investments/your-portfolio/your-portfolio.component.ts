...
export class YourPortfolioComponent implements OnInit {
  ...
  formValues: any;
  pendingRequestInfoText; /* For Translate text */
  ...
  getCustomerPortfolioDetailsById(customerPortfolioId) {
    this.manageInvestmentsService.getCustomerPortfolioDetailsById(customerPortfolioId).subscribe((data: any) => {
          ...
          "pendingRequestDTO": {
            ...
            "transactionDetailsDTO": [{
              ...
              },
              {
                ...
                "createdDate": 2567768503196,
                "expiryDate": 1567768403097,
                "transactionFrequency": "ONE_TIME",
                "transactionType": "BUY",
                "paymentMode": ""
              },
      ...
      this.pendingRequestInfoText = this.translate.instant(
        'YOUR_PORTFOLIO.PENDING_REQUEST_INFO',
        { count: this.portfolio.pendingRequestDTO.numberOfPendingRequests});
      ...
      const pendingBuyRequests = this.investmentEngagementJourneyService.findGroupByGroupName(buySellRequestGroups, 'BUY');
      const pendingSellRequests = this.investmentEngagementJourneyService.findGroupByGroupName(buySellRequestGroups, 'SELL');
      if (pendingBuyRequests && pendingBuyRequests.value) {
        this.pendingOnetimeBuyRequests = this.groupBuyRequests(pendingBuyRequests, 'ONE_TIME');
        this.pendingMonthlyBuyRequests = this.groupBuyRequests(pendingBuyRequests, 'MONTHLY');
        console.log(this.pendingMonthlyBuyRequests.processingRequests);
      }
    },
    (err) => {
