...
export class AccountCreationCommon {
  ...
  setThumbnail(thumbElem, file) {
    // Set Thumbnail
    const defaultThumb = ACCOUNT_CREATION_CONSTANTS.upload_documents.default_thumb;
  ...
  fileSelected(formData, control, controlname, fileElem, thumbElem?) {
    ...
    const isValidFileSize =
      fileSize <= ACCOUNT_CREATION_CONSTANTS.upload_documents.max_file_size;
    const isValidFileType = ACCOUNT_CREATION_CONSTANTS.upload_documents.file_types.indexOf(
      fileType
    ) >= 0;
