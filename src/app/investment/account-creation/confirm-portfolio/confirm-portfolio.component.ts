...
export class ConfirmPortfolioComponent implements OnInit {
  ...
  verifyCustomer() {
    ...
    if (investStatus && investStatus === SIGN_UP_CONFIG.INVESTMENT.ACCOUNT_CREATION_FAILED) {
      const pepData = this.accountCreationService.getPepData();
      const OldPepData = this.accountCreationService.getOldPepData();
  ...
  verifyAML() {
    if(!this.isRequestSubmitted) {
      ...
      const pepData = this.accountCreationService.getPepData();
      this.accountCreationService.verifyAML().subscribe(
        (response) => {
          ...
          if (response.responseMessage.responseCode < 6000) {
            ...
            response.objectList.status.toUpperCase() === ACCOUNT_CREATION_CONSTANTS.status.aml_cleared.toUpperCase() &&
            !pepData
          ) {
            this.createInvestmentAccount();
          } else {
  ...
  goToAdditionalDeclaration() {
    this.router.navigate([ACCOUNT_CREATION_ROUTE_PATHS.ADDITIONALDECLARATION]);
  }
  ...
  createInvestmentAccount() {
    if(!this.isRequestSubmitted) {
      ...
      this.accountCreationService.createInvestmentAccount().subscribe(
        (response) => {
          this.isRequestSubmitted = false;
          this.loaderService.hideLoader();
          if (response.responseMessage.responseCode < 6000) {
            ...
            if (response.objectList[response.objectList.length - 1]) {
              if (
                ...
                ACCOUNT_CREATION_CONSTANTS.status.account_creation_confirmed.toUpperCase()
              ) {
                this.accountCreationService.setAccountSuccussModalCounter(0);
                this.router.navigate([ACCOUNT_CREATION_ROUTE_PATHS.FUND_INTRO]);
              } else {
                this.accountCreationService.setAccountCreationStatus(
                  ACCOUNT_CREATION_CONSTANTS.status.account_creation_pending
                );
