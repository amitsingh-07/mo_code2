...
import {
    ACCOUNT_CREATION_ROUTE_PATHS, MY_INFO_START_PATH
...
export class SingPassComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.investmentData = this.accountCreationService.getInvestmentAccountFormData();
    ...
    this.myInfoSubscription = this.myInfoService.getMyInfoData().subscribe((data) => {
      if (data && data.objectList[0]) {
        ...
        if (window.location.href === redirectObjective) {
          this.router.navigate([ACCOUNT_CREATION_ROUTE_PATHS.START], { skipLocationChange: true }).then(() => {
  ...
  getMyInfo() {
    this.showConfirmation = false;
    this.accountCreationService.setCallBackInvestmentAccount();
    this.myInfoService.setMyInfoAttributes(
      this.accountCreationService.myInfoAttributes
    );
