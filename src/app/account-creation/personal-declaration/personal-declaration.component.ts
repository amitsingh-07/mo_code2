...
export class PersonalDeclarationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getSourceList();
    this.personalDeclarationFormValues = this.accountCreationService.getPersonalDeclaration();
  ...
  goToNext(form) {
    if (!form.valid) {
      ...
      return false;
    } else if (this.accountCreationService.setPersonalDeclarationData(form.getRawValue())) {
      this.saveInvestmentAccount();
    }
  }

  saveInvestmentAccount() {
    this.accountCreationService.saveInvestmentAccount().subscribe(
      (data) => {
        if (this.accountCreationService.getMyInfoStatus()) {
