...
export class AdditionalDeclaration1Component implements OnInit {
  ...
  addOtherOccupation(value) {
    if (
      value.occupation === ACCOUNT_CREATION_CONSTANTS.ADDITIONAL_DECLARATION_ONE.OTHERS
    ) {
      this.addInfoForm.addControl(
        'pepOtherOccupation',
  ...
  goToNext(form) {
    if (!form.valid) {
      ...
      this.accountCreationService.setAdditionalInfoFormData(form.getRawValue());
      this.router.navigate([
        ACCOUNT_CREATION_ROUTE_PATHS.ADDITIONAL_DECLARATION_SCREEN_2
      ]);
    }
  }
